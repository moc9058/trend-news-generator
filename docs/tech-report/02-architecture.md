...
2. README のフロー①〜④(収集 → 生成・自動投稿 → 承認公開 → トークン更新)と、随時使うリサーチチャットが、どの部品をどの順に通って実行されるか(§4 のシーケンス図 5 枚)
...
| パイプラインのジョブ群 | Cloud Run Jobs ×7(§3 で個別に示す) | `pipeline/app/jobs/`。デプロイは `infra/10-deploy-pipeline.sh` |
...
次に GCP の中を開く。**Cloud Run** は GCP のコンテナ実行サービスで、HTTP を待ち受ける常駐型の「サービス」と、起動されて処理を終えると止まる「ジョブ」の 2 種類がある。本システムはサービス 2 つ+ジョブ 7 つ。**サービスアカウント(SA)**はプログラム専用の Google アカウントで、3 つを役割別に使い分ける(矢印のラベルに付記)。
...
flowchart TB
    ...
    subgraph GCP["GCP プロジェクト trend-news-generator / asia-northeast1"]
        ...
        JOBS["Cloud Run Jobs ×7<br/>実行 SA: pipeline-sa"]
...
図の読み方: 実線は実行時の呼び出し、点線は起動時の供給(イメージとシークレット)。定時実行は Cloud Scheduler(指定時刻に HTTP リクエストを送るタイマー)が scheduler-sa の認証でジョブを蹴り、人間の操作は IAP(Identity-Aware Proxy、Google ログインを強制する門番)を通って admin-ui に届く。admin-ui が pipeline-api を呼べるのは、admin-sa が pipeline-api の呼び出し権限(`run.invoker`)を持つからで、逆に他の誰も呼べない。図で省略した線が 3 本ある — admin-sa は GCS の読み取り(画像プレビュー用)もでき、pipeline-api はジョブと同じ pipeline-sa で動くため GCS・Secret Manager にも同様に触れる。また scheduler-sa の権限はスケジュール対象の 6 ジョブに限られ、job-seed(初期データ投入)にはスケジューラ自体がない。ロールの網羅表は [04-parameters.md](04-parameters.md) の §6 が正。
...
| job-generate-short | Cloud Run ジョブ | フロー② 短文の生成と自動投稿 | 同上 |
| job-generate-article | Cloud Run ジョブ | フロー② 記事(長文)の下書き生成(投稿はしない)。レポートは Research Agent(doc 10、後続フェーズ) | 同上 |
| job-cleanup-drafts | Cloud Run ジョブ | 未承認の下書きを30日で自動削除 | 同上 |
...
## 4. フロー別シーケンス図 — README の①〜④の実行経路 + チャット

README の表にあるフロー①〜④を、時間の流れに沿ったシーケンス図(登場者を縦線で並べ、上から下へ時系列にやり取りを示す図)で 1 本ずつ示す。最後の §4.5(リサーチチャット)は定時フローではなく**運用者が随時使う対話経路**で、唯一のストリーミング経路でもあるため併記する。
...
### 4.2 フロー② 短文生成と自動投稿 — job-generate-short

```mermaid
sequenceDiagram
    participant SCH as Cloud Scheduler
    participant JOB as job-generate-short
    participant FS as Firestore
...
毎朝の投稿。カテゴリごとに LLM を 1 回だけ呼んで X・Threads・Notion の 3 本文をまとめて受け取り、文字数を検査してから投稿(post)を作る。既定では承認不要(`approved`)なので、**同じジョブのプロセス内で**そのまま `publish_post()` が呼ばれ、notion → x → threads の固定順(§7.4)で公開まで進む。運用フラグ `shortRequireApproval` を有効にすると下書き(`draft`)で止まり、フロー③と同じ承認経路に乗る。記事(article、月曜 07:00)の長文ジョブもこの図の「生成 → posts 作成」までは同型だが、2 段階生成(軽量モデルで素材選定 → 上位モデルで執筆)であること、そして**必ず `draft` で止まり投稿処理を一切呼ばない**ことが違う(旧 monthly の深掘り調査はレポート=Research Agent に置換、doc 10)。生成の詳細は [05-detailed-design/03-generate.md](05-detailed-design/03-generate.md)、投稿の詳細は [05-detailed-design/04-publish.md](05-detailed-design/04-publish.md)。

| 名前 | 実体 | 定義場所 |
|---|---|---|
| Cloud Scheduler | `sched-generate-short`(記事は別スケジューラ) | `infra/20-schedulers.sh` |
| job-generate-short | Cloud Run ジョブ | 本体は `pipeline/app/jobs/generate_short.py`、生成ロジックは `pipeline/app/generators/short.py` |
...
| Secret Manager | シークレット `threads-access-token` のバージョン群 | 作成は `infra/01-secrets.sh`、権限付与は `infra/10-deploy-pipeline.sh` |
| Firestore | `settings/channelHealth` と `runs` | [03-data-model.md](03-data-model.md) |

### 4.5 フロー⑤ リサーチチャット — 唯一のストリーミング経路

```mermaid
sequenceDiagram
    participant OP as 運用者(ブラウザ)
    participant IAP as IAP
    participant RH as admin-ui route handler<br/>/api/chat/stream
    participant API as pipeline-api<br/>POST /api/chat/messages
    participant WK as ワーカースレッド(グラフ)
    participant EXT as 外部ソース / OpenAI
    participant FS as Firestore

    OP->>IAP: fetch POST(質問・モード・深さ)
    IAP->>RH: x-goog-authenticated-user-email 付きで転送
    RH->>RH: requestedBy を注入 + OIDC ID トークン取得
    RH->>API: POST(body)
    API->>FS: user メッセージ保存 / assistant を streaming で作成
    API->>WK: グラフ実行を開始(別スレッド)
    API-->>RH: SSE: meta
    RH-->>OP: body をそのまま素通し
    WK->>EXT: 検索 → 本文取得 → 統合(調査モード)
    WK-->>API: status / token / sources をキュー経由で
    API-->>OP: SSE: status / token / sources(15秒無音なら : ping)
    WK->>FS: 増分保存(≥1.5秒間隔)
    WK->>FS: 最終確定(content / sources / usage / status)
    API-->>OP: SSE: usage → done
```

他の 4 フローと違い、**応答を1つ返して終わらない**唯一の経路。3 点が構造を決めている:

1. **admin に route handler が1つだけ存在する理由** — Server Action は値を1つ返す仕組みなのでストリームを返せない。ブラウザが fetch できる URL が要る。役割は IAP 由来の `requestedBy` と ID トークンの注入のみで、body は変換せず素通しする(パースした時点でバッファされ、ストリーミングが死ぬ)。
2. **グラフを別スレッドで回す理由** — グラフは同期実装で、FastAPI のスレッドプールを数分占有してしまう。加えて**ブラウザを閉じても run は完走**し Firestore に書き切る(リロードすれば完全な回答が出る)。
3. **Cloud Run 設定は変更不要** — deep でも上限 10 分 < pipeline-api の timeout 900 秒。ジョブもスケジューラも増えない。

詳細は [05-detailed-design/11-research-chat.md](05-detailed-design/11-research-chat.md)。
...
インフラは番号順に実行する 5 本のシェルスクリプト(`infra/00` → `01` → `10` → `11` → `20`)で作る。サービス 2 つ+ジョブ 7 本という規模の個人運用では、状態管理ツール(Terraform 等)を持ち込む学習・保守コストより「読めば分かる冪等スクリプト」の単純さが勝つ、という判断。各スクリプトは再実行しても安全に同じ状態へ収束するよう書かれている。読解は [05-detailed-design/08-infra.md](05-detailed-design/08-infra.md)。
...
README の図は要約ゆえに単純化がある(例: 生成ジョブを 1 ノードに束ねる、pipeline-api への SA 線を省く)。構成に関する疑問はまず本書で確認し、実装レベルの疑問は [05-detailed-design/](05-detailed-design/01-pipeline-foundation.md) の該当章へ進むこと。
