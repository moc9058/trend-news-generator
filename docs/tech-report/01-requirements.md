...
- 直近 36 時間・最大 15 件の item を材料に、`gpt-5.4-mini`(既定)への 1 回の JSON モード呼び出しで X 用・Threads 用・Notion 用のテキストを同時に得る(チャネル別に呼ばないため呼び出し回数は 1/3。文字数超過時のみ縮小リトライで +1 回)。
...
- 第 1 段(選定): 週次は 7 日・月次は 30 日の候補 item(最大 120 件、タイトル+要約のみ)を `gpt-5.4-mini` に渡し、テーマ・章立て・使用 item の ID を選ばせる。候補が 3 件未満のカテゴリはスキップ。
- 第 2 段(執筆): 選定された item(最大 25 件、各本文最大 4,000 字)を `gpt-5.5`(既定)に渡し、本文(Markdown)・要約・X / Threads 用ティーザー(本文へ誘導する短い紹介文)を受け取る。
...
- 全 10 画面(ダッシュボード/下書き一覧・編集/投稿履歴/カテゴリ/ソース/プロンプト一覧・編集/チャネル/設定)。
- 書き込み 12 操作のうち、DB 内で完結する 9 操作(設定保存・下書きテキスト保存)は Firestore へ直接書き、外部への副作用がある 3 操作(承認公開・リトライ・ジョブ実行)だけが pipeline-api を経由する。
...
- 6 ジョブ(`collect` / `generate_daily` / `generate_weekly` / `generate_monthly` / `refresh_threads_token` / `seed`)を pipeline-api の `POST /api/jobs/{name}/run` で起動できる。
...
- 当月の LLM コスト概算(`runs` の `costUsd` を UTC 月で合計)、Threads トークンの残り日数(14 日未満で赤字)、承認待ち下書き数の 3 カードを表示する。
- Threads トークン更新の失敗時は画面最上部に赤帯が出る(`threadsRefreshError` が空でないとき)。
- 承認待ち下書き(先頭 5 件)・直近の投稿(10 件、チャネル別状態バッジ付き)・ジョブ実行履歴(10 件、成否・統計・先頭エラー)を一覧する。
...
- **LLM**: 長文生成は「安い `gpt-5.4-mini` で選定 → 高い `gpt-5.5` は絞り込んだ入力で執筆のみ」の 2 段階で入力トークンを桁で節約する(FR-2.2)。日次は 1 回の呼び出しで 3 チャネル分を得る(FR-2.1)。
- **可視化**: 実測トークン数からのコスト概算を投稿(`tokenUsage`)と実行記録(`runs.costUsd`)に保存し、ダッシュボードで当月合計を表示する。単価表はコード内固定の**目安**であり、実請求は OpenAI ダッシュボードで確認する運用([../runbook.md](../runbook.md))。
...
| FR-2.1 | `pipeline/app/generators/daily.py`(入口 `pipeline/app/jobs/generate_daily.py`) | [05-detailed-design/03-generate.md](05-detailed-design/03-generate.md) |
...
| FR-3.2 | `pipeline/app/generators/daily.py`(初期状態の分岐)、`pipeline/app/main.py`(承認 API)、`admin/src/components/DraftEditor.tsx` | [05-detailed-design/05-pipeline-api.md](05-detailed-design/05-pipeline-api.md) |
...
| 10 | データの保持期間管理 | `items` / `posts` / `runs` の自動削除・アーカイブは無い。全コレクションの書き手一覧([03-data-model.md](03-data-model.md))に削除処理は存在しない(手動削除は可能) |
