...
| `channelConfigs` | `{categoryId}_{cadence}_{channel}` | seed・admin | 生成ジョブ・admin | カテゴリ×カデンス×チャネル単位の有効/言語設定 |
| `promptTemplates` | `{categoryId}_{cadence}` | seed・admin | 生成ジョブ・admin | 生成プロンプトのテンプレート |
| `settings`(doc `app`) | 固定名 `app` | seed・admin | 生成・公開処理・admin | 全体設定(日次の承認要否・画像添付など) |
...
| daily 生成 | `pipeline/app/generators/daily.py`(入口は `pipeline/app/jobs/generate_daily.py`) |
| longform 生成 | `pipeline/app/generators/longform.py`(入口は `pipeline/app/jobs/generate_weekly.py` / `generate_monthly.py` → `longform_runner.py`) |
...
| フィールド | 型 | 意味 | 書き手 | 読み手 |
|---|---|---|---|---|
| `cadence` | string | `daily`(日次)/ `weekly`(週次)/ `monthly`(月次) | 生成 | admin 読、`recent_by_cadence()` |
...
| `summary` | string | 記事要約 | 生成、admin 書 | publish(body 空時の代替)、longform 生成(monthly が weekly の要約を読む)、admin 読 |
...
| `createdAt` | timestamp | 作成日時(`posts.create()` が付与) | `repo/posts.py` | admin 読(一覧の並び順)、longform 生成(31 日窓判定) |
...
| `threadParts` | array<string> | X のスレッド分割(daily で 2 分割以上になったときのみ非空) | daily 生成 | publish(`x.publish()` の thread_parts) |
...
| `imageGcsPath` | string | X/Threads 添付画像の GCS パス(daily 生成のみ設定) | daily 生成 | publish(X はバイト列添付、Threads は署名 URL) |
...
具体例(`posts/(自動採番ID)`、daily が全チャネル成功した状態。一部フィールドは省略):
...
| フィールド | 型 | 意味 | 書き手 | 読み手 |
|---|---|---|---|---|
| `jobType` | string | `collect` / `generate_daily` / `generate_weekly` / `generate_monthly` / `refresh_threads_token` | 各ジョブ | admin 読 |
...
出典クラス: `pipeline/app/models.py` の `ChannelConfig`。docID = `{categoryId}_{cadence}_{channel}`(例 `science-technology_daily_x`)。seed は 3 カテゴリ × 3 カデンス × 3 チャネル = 27 件を、既定言語 x=`ja` / threads=`ko` / notion=`en`(`seed.py` の `CHANNEL_LANGUAGES`)で投入する。生成ジョブは投稿を作る瞬間にこれを読み、`ChannelState` の `enabled` / `lang` に複製する。

| フィールド | 型 | 意味 | 書き手 | 読み手 |
|---|---|---|---|---|
| `categoryId` / `cadence` / `channel` | string | docID の 3 要素と同じ値(検索できるよう本体にも重複保存) | seed、admin 書 | admin 読(一覧) |
| `enabled` | boolean | この組み合わせで投稿するか | seed、admin 書 | daily/longform 生成(`configs.channel_config()`) |
| `language` | string | 出力言語(`ja` / `ko` / `en`) | seed、admin 書 | daily/longform 生成(プロンプトの言語指定に変換) |
...
具体例(`channelConfigs/science-technology_daily_x`): `{ "categoryId": "science-technology", "cadence": "daily", "channel": "x", "enabled": true, "language": "ja" }`

### promptTemplates

出典クラス: `pipeline/app/models.py` の `PromptTemplate`。docID = `{categoryId}_{cadence}`(例 `science-technology_weekly`)。seed が `pipeline/app/generators/prompts.py` の `DEFAULTS` から 3 カテゴリ × 3 カデンス = 9 件を投入する。`enabled=false` にすると `configs.prompt_template()` が None を返し、そのカテゴリ×カデンスの生成はスキップされる(生成の詳細は `05-detailed-design/03-generate.md`)。
...
| `dailyRequireApproval` | boolean | true なら日次も `draft`(下書き)で止める安全弁(既定 false = 自動投稿) | seed、admin 書 | daily 生成(初期 status の分岐) |
| `xAllowUrlOnDaily` | boolean | 日次の X 投稿にも Notion URL を付けるか(既定 false。X は URL 付きだとリーチが下がる想定の運用判断) | seed、admin 書 | publish(`_publish_x()`) |
| `attachImages` | boolean | X/Threads への画像添付を行うか(既定 true) | seed、admin 書 | daily 生成(画像選択)、publish(`_load_image()` / 署名 URL 発行) |
...
| `promptTemplates` | `{categoryId}_{cadence}` | 下記参照 |
| `channelConfigs` | `{categoryId}_{cadence}_{channel}` | 下記参照 |
...
- pipeline 側
  - `pipeline/app/repo/configs.py` の `prompt_template()`(`f"{category_id}_{cadence.value}"`)と `channel_config()`(`f"{category_id}_{cadence.value}_{channel.value}"`)
  - `pipeline/app/jobs/seed.py`(初期投入時に同じ規則で docID を組み立てる)
- admin 側
  - `admin/src/app/[locale]/channels/page.tsx`(`` `${cat.slug}_${cadence}_${channel}` `` を組み立てて `saveChannelConfig()` に渡す)
  - `admin/src/app/[locale]/prompts/page.tsx`(`` `${cat.slug}_${cadence}` `` を組み立てて編集ページへのリンクにする)
  - `admin/src/app/[locale]/prompts/[id]/page.tsx`(**逆方向の依存**: URL 中の docID を最後の `_` で分解して categoryId と cadence を復元する)

なお最後の「分解」は `id.lastIndexOf('_')` を使うため、カテゴリ slug にアンダースコアが混ざっても壊れはしないが、cadence 側に `_` を含む値を追加すると壊れる。カテゴリ slug はハイフン区切り(seed の 3 件はすべてそう)を守るのが安全である。
...
図の読み方: 投稿の入口は 2 つある。weekly/monthly は必ず `draft` で作られ、admin での承認を待つ。daily は `settings/app` の `dailyRequireApproval` が false(既定)なら最初から `approved` で作られ、`generate_daily` ジョブがそのまま `publish_post()` を呼ぶ — これが「日次は自動投稿」の実体である。同フラグが true のときだけ daily も `draft` で止まり、weekly/monthly と同じ承認フローに乗る。終端は 3 種類で、公開後に失敗チャネルだけをやり直すと再び `publishing` を経由する。

| 遷移 | トリガー | 実行コード |
|---|---|---|
| (生成) → `draft` | weekly/monthly の生成。daily は `dailyRequireApproval=true` のときのみ | `generators/longform.py` / `generators/daily.py` の `generate_for_category()`(保存は `repo/posts.py` の `create()`) |
| (生成) → `approved` | daily の生成(承認不要設定・既定) | `generators/daily.py` の `generate_for_category()` |
...
Firestore では「複数フィールドを組み合わせた絞り込み+並べ替え」に複合インデックス(検索を高速化するための事前の並び順定義)が必要で、無いとクエリ自体がエラーになる。定義は `infra/firestore.indexes.json` に 5 件(デプロイ手順は `05-detailed-design/08-infra.md`)。各定義と、それを必要とするクエリの対応:

| # | コレクション | フィールド構成 | 対応するクエリ | 用途 |
|---|---|---|---|---|
| 1 | `items` | `categoryId` ASC, `collectedAt` DESC | `pipeline/app/repo/items.py` の `recent_for_category()` | 生成の元ネタ取得(daily=36 時間窓・最大 15 件、weekly=7 日・monthly=30 日窓・最大 120 件) |
...
| 4 | `posts` | `cadence` ASC, `createdAt` DESC | `pipeline/app/repo/posts.py` の `recent_by_cadence()` | monthly 生成が当月の weekly 記事要約を読む(階層的集約) |
...
| 変更の種類 | 同時に触るファイル |
|---|---|
| enum 値の追加・変更(cadence / channel / status / sourceType / jobType / language) | `shared/constants.json`(唯一のソース)→ `pipeline/app/models.py` の対応 Enum(**手書きミラー**。pipeline は実行時に constants.json を読まない)→ admin 再ビルド(prebuild の `node scripts/sync-constants.mjs` が `admin/src/lib/shared-constants.json` へコピー)→ 本文書 |
