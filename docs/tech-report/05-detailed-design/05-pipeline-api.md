...
- Cloud Run サービス `pipeline-api`(FastAPI 製の小さな Web API)が持つ全 4 エンドポイントの仕様と、管理画面(admin)からの呼ばれ方
- コードを読むと「無認証」に見えるこの API が、実際にはインフラ層(Cloud Run IAM)でどう守られているか
- ジョブの手動実行が「受け付けたら即応答」になっている仕組み(BackgroundTasks)と、実行結果をどこで確認するか
...
なお、pipeline-api と 6 つの Cloud Run Jobs は**同一の Docker イメージ**である。`pipeline/Dockerfile` の既定の起動コマンドは `uvicorn app.main:app`(= この API)で、ジョブは `infra/10-deploy-pipeline.sh` が `--command=python --args=-m,app.jobs.<name>` で起動コマンドだけ差し替えている。API とジョブでコードのずれが起きない構成になっている。
...
docstring の最後の一文も設計上重要だ。投稿一覧や設定値など**表示のための読み書きは、管理画面が firebase-admin で Firestore を直接操作する**([07-admin-ui.md](07-admin-ui.md))。pipeline-api を通るのは「DB 更新にとどまらない副作用がある操作」= 外部 SNS への公開・再試行・ジョブ起動の 3 つだけ。だからエンドポイントは 4 つ(うち 1 つは死活確認)しかない。
...
`name` は `JOB_MODULES` に定義された 6 種のみ:
...
- `name not in JOB_MODULES` — 許可された 6 種以外の名前は 400。`JOB_MODULES` はモジュール実行には使わなくなったが、**許可リスト(ホワイトリスト)**として残している。
...
- 3 つの公開関数 `publishPost()` / `retryChannel()` / `runJob()` はすべてこの `call()` を通り、応答を `{ ok: resp.ok, detail: 本文 }` に整形して Server Action に返す。
