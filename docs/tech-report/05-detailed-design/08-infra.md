...
**③ 複合インデックス** — Firestore は「categoryId で絞り込み、collectedAt の新しい順に並べる」のような**複数フィールドを組み合わせたクエリ**に、事前定義の複合インデックスを要求します。スクリプト内の `create_index` 関数は `gcloud firestore indexes composite create` の共通部分をくくり出したもので、第 1 引数がコレクション名、`"${@:2}"`(2 番目以降の全引数)が `--field-config=field-path=…,order=…` の列としてそのまま渡ります。`--query-scope=COLLECTION` は「単一コレクション内のクエリ用」、`--async` は「作成完了を待たずに次へ進む」(インデックス構築は数分かかるため)という意味です。作られる 7 本は `items`×2(カテゴリ別新着一覧、タイトル近似重複の 7 日窓チェック用)、`posts`×2(管理画面のステータス別・format 別一覧用)、`sources`×1(カテゴリ内の有効ソース絞り込み用)、`researchRuns`×1(レポート調査 run のキュー取得用)、`chatThreads`×1(リサーチチャットのスレッド一覧用)。同じ定義が `infra/firestore.indexes.json` にもあり、これは Firebase CLI(`firebase deploy --only firestore:indexes`)でも同じインデックスを張れるように残したミラーです。**両者は手で同期する**必要があります。

> **この「手で同期」は実際に落とし穴になりました**(2026-07-15)。リサーチチャットのスレッド一覧クエリが複合インデックスを要るのに両方へ追加し忘れ、本番の `/chat` が 500(`9 FAILED_PRECONDITION`)になりました。**コレクションが空でも、クエリの形だけで失敗します**。ローカルのテストは Firestore をモックするため検知不能なので、新しいクエリを書いたら [`../03-data-model.md`](../03-data-model.md) §6 の表で確認するのが唯一の防御線です。
