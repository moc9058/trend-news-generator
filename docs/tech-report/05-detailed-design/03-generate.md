...
- 毎日 08:00(JST)の `job-generate-short`(旧 daily)が、収集アイテム(item)から X・Threads・Notion 向けの短文投稿(post)を LLM 1回の呼び出しで作り、承認不要なら即投稿するまでの仕組み。
- 月曜 07:00 の `job-generate-article`(旧 weekly)が、「安いモデルで素材選定 → 高性能モデルで本文執筆」という2段階で長文記事の下書き(`draft`)を作る仕組み。レポート(report、旧 monthly の深掘り調査)は Research Agent に置換され、本書ではなく [10-research-agent.md](10-research-agent.md) が扱う。
...
| ファイル | 役割 |
| --- | --- |
| `pipeline/app/jobs/generate_short.py` | 短文ジョブ本体。カテゴリごとに短文生成 → 保存 → (承認不要なら)即投稿 |
| `pipeline/app/jobs/generate_article.py` | 記事ジョブの入口。`run_longform(Format.article)` を呼ぶだけの薄いラッパー |
| (旧 `generate_monthly.py`) | 削除。レポートは Research Agent(`jobs/generate_report.py`、[10-research-agent.md](10-research-agent.md)、後続フェーズ)に置換 |
| `pipeline/app/jobs/longform_runner.py` | 記事のジョブ骨格(カテゴリループ・保存・実行記録) |
| `pipeline/app/generators/short.py` | 短文の生成ロジック(1回の LLM 呼び出しで3チャネル分を JSON 生成) |
| `pipeline/app/generators/longform.py` | 記事(長文)の2段階生成ロジック |
...
| `pipeline/app/repo/posts.py` | post の読み書き(作成・状態更新ほか) |
...
### (a) 短文(job-generate-short、毎日 08:00 JST)
...
短文はカテゴリごとに LLM を1回だけ呼び、X 用・Threads 用・Notion 用の3テキストをまとめて JSON で受け取ります。文字数を検査し、超過していれば「短く書き直せ」というリトライを1回、それでも Threads が長ければ機械的に切り詰めます。アプリ設定 `shortRequireApproval` が無効(既定)なら post は `approved` で作られ、その場でジョブ内から投稿まで進みます。有効なら `draft` で止まり、管理画面での承認を待ちます。

### (b) 記事(長文)(job-generate-article)
...
### 4.1 短文(`generate_short.main()` → `short.generate_for_category()`)
...
- **役割**: 短文ジョブのエントリポイント。`python -m app.jobs.generate_short` で起動される。
- **入出力**: 引数・戻り値なし。副作用として `posts` / `items` / `runs` を更新し、承認不要時は外部 SNS へ投稿する。
- **呼び出し元・先**: Cloud Run Job `job-generate-short`(Cloud Scheduler `0 8 * * *` JST)から起動 → `configs.enabled_categories()`、`short.generate_for_category()`、`posts.create()`、`items.mark_used()`、`publish_post()`、`runs.start()/finish()`。
...
### short.generate_for_category()

- **役割**: 1カテゴリ分の短文 post を作る中核関数。
- **入出力**: `Category` を受け取り、`Post | None` を返す(スキップ時 `None`。保存は呼び出し元)。
- **呼び出し元・先**: `generate_short.main()` から → `configs.channel_config()/prompt_template()/app_settings()`、`items.recent_for_category()`、`prompts.format_items_for_prompt()`、`generate_json()`、`_shrink_retry()`、`renderer.strip_urls()/fits_x()/fits_threads()/split_for_x_thread()`。
...
- **役割**: 記事のジョブ骨格。短文の `main()` から「即投稿」を除いた形。
- **入出力**: `Format`(`article`)を受け取る。戻り値なし。
- **呼び出し元・先**: `generate_article.main()` から → `longform.generate_for_category()`、`posts.create()`、`items.mark_used()`、`runs.start()/finish()`。
- **要点**: `job_type = f"generate_{post_format.value}"` として `runs` に記録する(記事は `generate_article`)。`publish_post` を呼ぶコードパスが存在しない=長文が誤って自動投稿される余地が構造的にない。

### longform.generate_for_category()

- **役割**: 1カテゴリ分の長文下書きを2段階生成で作る中核関数。
- **入出力**: `Category` と `Format`(`article`)→ `Post | None`(常に `status=draft`)。
- **呼び出し元・先**: `run_longform()` から → `configs.*`、`items.recent_for_category()/get_many()`、`prompts.format_items_for_prompt()`、`generate_json()`(2回)。
- **外部アクセス**: Firestore、OpenAI API(Stage1 と Stage2 で計2回)。
- **要点**: Stage1 は `openai_model_daily`、Stage2 は `modelOverride or openai_model_longform` と、**`modelOverride` が効くのは Stage2 だけ**。Stage1 のプロンプトは `outlineSystemPrompt` / `outlineUserPromptTemplate` が空だと `prompts.ARTICLE_OUTLINE_SYSTEM` と `"{items}"` に落ちる。(旧 monthly の「週次まとめ集約」`_weekly_summaries_for_month()` は削除済み — レポートは Research Agent、[10-research-agent.md](10-research-agent.md)。)
...
- **呼び出し元・先**: 短文(ID なし・要約のみ)、長文 Stage1(ID あり・要約のみ)、長文 Stage2(ID あり・`max_content=4000` で全文添付)から。
...
| `configs.prompt_template()` | `promptTemplates/{category}_{format}` を読む。**ドキュメントが無い、または `enabled=false` なら `None`**(=そのカテゴリをスキップ) |
| `configs.channel_config()` | `channelConfigs/{category}_{format}_{channel}` を読む。無ければ「無効・英語」の既定値を返す |
| `configs.app_settings()` | `settings/app`(`shortRequireApproval` / `attachImages` など)を読む |
...
- `usage: TokenUsage` — 戻り値で返すのではなく、呼び出し側が持つ入れ物に**加算**する設計。短文の「初回+縮小リトライ」や長文の「Stage1+Stage2」のように複数回呼んでも、post 1件の合計トークン・合計コストが自動的にまとまる。
...
X は単純な文字数ではなく「重み付き文字数」(全角=2、半角=1、URL は一律23)で280が上限、Threads は素の500字が上限です(`pipeline/app/publishers/renderer.py`)。LLM は「250字以内で」と指示しても超えることがあるため、短文は3段構えで守ります。
...
- **第1段(予防)**: プロンプト自体が X 250重み字・Threads 480字と、上限より2〜4%厳しい値を指示(`prompts.SHORT_USER`)。`strip_urls()` は生成文に紛れ込んだ URL を除去する(URL 1つで重み23を食うため、短文 X は URL を載せない方針)。
- **第2段(縮小リトライ)**: どちらかが超過したら `_shrink_retry()` を**1回だけ**。前回の出力を丸ごと見せて「同じ JSON をもっと短く」と依頼するので、内容を保ったまま縮む可能性が高い。リトライは追加コストなので無限には行わない。
- **第3段(ハードトリム)**: それでも Threads が超過したら `THREADS_LIMIT - 1`(=499字)で切って「…」を付ける。文としては不格好でも、**短文ジョブが文字数だけを理由に落ちることは絶対にない**という保証を優先した「last resort」。
...
なお記事のティーザーはこの検査を通りません。長文は人間の承認を挟むうえ、公開時の URL 追記で `renderer.append_url()` が上限に収まるまで本文を削るため、生成段階では厳密でなくてよいという割り切りです。
...
プロンプトは「コード内の既定値」と「Firestore 上の実体」の2層です。`pipeline/app/generators/prompts.py` の `DEFAULTS`(short は system+user、article/report はさらに outline 用2種)は、seed ジョブが初回に `promptTemplates/{category}_{format}` として Firestore へ書き込むための**雛形**であり、**実行時に読まれるのは常に Firestore 側**です。管理画面での編集は Firestore を書き換えるだけで即反映され、コードのデプロイは不要です。
...
- `.format(items=..., category=..., date=..., language=...)` — テンプレート本文の `{items}` などのプレースホルダに実データを流し込む。短文はさらにチャネル別言語(`{x_language}` `{threads_language}` `{notion_language}`)、長文 Stage2 は `{theme}` `{outline}` が使える。**管理画面でテンプレートを編集する際、その format で渡されないプレースホルダ(例: 短文に `{theme}`)を書くと `KeyError` になり、そのカテゴリの生成が失敗する**(§7 の隔離で他カテゴリには波及しない)。
...
- **カテゴリ単位の隔離**: `generate_short.main()` と `run_longform()` はどちらも、生成中の例外を `try/except` で捕まえて `run.errors` に `"generate {slug}: {原因}"` を追記し、**次のカテゴリの処理を続行**します。1カテゴリのプロンプト不備や API エラーで全カテゴリが道連れになることはありません。短文の投稿失敗(`publish {postId}: ...`)も同様に記録して続行します。
- **スキップ条件(エラーではなく正常系)**: 短文は「3チャネルすべて無効」「直近36時間の item 0件」「テンプレート無し/無効」で、長文は「テンプレート無し/無効」「候補 item 3件未満」で、その回のそのカテゴリを黙って(ログには残して)スキップします。スキップは `run.errors` に載らず `ok` にも影響しません。
...
- **ジョブ全体が落ちた場合**: 短文・記事の生成ジョブは Cloud Run Job の `--max-retries=0` でデプロイされており(短文はジョブ内で投稿するため二重投稿防止。`01-pipeline-foundation.md`)、自動再実行されません。`runs` に `finishedAt` の無いレコードが残るのがサインです。なお後続フェーズのレポート生成ジョブ(`generate-report`)は投稿せず lease/resume で二重実行を防ぐため `--max-retries=1`(doc 10 §6.3)。
...
| テスト | 生成フローとの関係 |
| --- | --- |
| `pipeline/tests/test_renderer.py` | 短文が使う文字数規則そのもの。全角=2/URL=23 の重み、`fits_x` / `fits_threads` の境界、`split_for_x_thread` の連番と各パートの上限、`strip_urls`、`append_url` の切り詰めを検証 |
| `pipeline/tests/test_publish_orchestration.py` | 生成が作った `Post` の「契約」を下流側から検証。短文 X に URL が付かないこと、記事ティーザーに Notion URL が追記されること、`skipped` チャネルの扱いなど |
| `pipeline/tests/test_api.py` | pipeline-api 経由のジョブ起動・公開エンドポイントの入口を検証 |

つまり「LLM が何を書くか」は守れないが、「書かれたものが各チャネルの制約に収まる/収まるように加工される」ことと「post の形が公開処理と噛み合う」ことは間接的に守られています。生成ロジックを変更したら、最低限 `cd pipeline && pytest` で回帰がないことを確認し、決定的な部分(例: `format_items_for_prompt()` の整形、`renderer` の文字数検査)を触る場合はテスト追加を検討してください。
...
| 特定カテゴリだけモデルを変える | 管理画面で該当 `promptTemplates` の `modelOverride` を設定 | 効くのは短文と長文 Stage2 のみ。Stage1 は常に `openai_model_daily` |
| プロンプトの文面変更 | 運用中は**管理画面**(Firestore の `promptTemplates` を直接編集、即反映) | `prompts.py` の `DEFAULTS` は seed 時の雛形にすぎず、変更しても既存ドキュメントには反映されない(新カテゴリの seed にだけ効く)。プレースホルダはその format で渡されるものだけを使う(§6.4) |
| 文字数制限の変更 | `pipeline/app/publishers/renderer.py`(`X_LIMIT` / `THREADS_LIMIT`)**と** `admin/src/lib/textLimits.ts` の同名定数 | 2箇所は手動ミラー。さらに `prompts.py` の既定プロンプト内の指示値(250/480、ティーザー200/400)と `short._shrink_retry()` の文言(250/480)も揃えること |
...
スケジュール(08:00 短文 / 月曜 07:00 記事)を変えたい場合は `infra/20-schedulers.sh` の cron 式を編集して再実行します(`01-pipeline-foundation.md`)。
