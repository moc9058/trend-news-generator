...
全画面は `layout.tsx` の共通枠の中に表示される。共通枠は濃紺(ink)の固定サイドバーで、8 項目のナビゲーションを運用フロー順の 4 グループ(モニタリング / コンテンツ / パイプライン設定 / システム)に分けて表示する(グループラベルは `messages/*.json` の `nav.group*` キー)。各項目はアイコン付きで、現在表示中の画面が `NavLink.tsx`(`usePathname()` で URL と照合)によりハイライトされる。サイドバー下部には IAP 認証済みユーザーのメールアドレス(`iapUserEmail()`)と言語切替を表示する。画面幅が狭いとき(`lg` ブレークポイント未満)はサイドバーがアイコンのみの幅に自動縮小する。
...
一方、クリックや入力に反応するコードはブラウザで動く必要がある。ファイル先頭に `'use client'` と書いたものだけがブラウザに JavaScript として配送される(Client Component)。admin でこの宣言を持つのは **`DraftEditor.tsx` / `ActionButton.tsx` / `LocaleSwitcher.tsx` / `NavLink.tsx` の 4 ファイルだけ**(NavLink は `usePathname()` で現在 URL を読むため)。`ui.tsx` と `icons.tsx` には宣言がなく、サーバ・ブラウザ両方から import されて双方で使える。原則: **表示とデータ取得はサーバ、対話だけブラウザ**。
