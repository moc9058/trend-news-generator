...
> 対象コード時点: コミット e93a089 + 未コミット変更 / 最終更新: 2026-07-13
...
全画面は `layout.tsx` の共通枠の中に表示される。共通枠のレイアウトは `AppShell.tsx`(Client Component)が描く。濃紺(ink)のサイドバーに 8 項目のナビゲーションを運用フロー順の 4 グループ(モニタリング / コンテンツ / パイプライン設定 / システム)に分けて表示する(グループラベルは `messages/*.json` の `nav.group*` キー。翻訳は `layout.tsx` がサーバ側で解決し props で渡す)。各項目はアイコン付きで、現在表示中の画面が `NavLink.tsx`(`usePathname()` で URL と照合)によりハイライトされる。サイドバー下部には IAP 認証済みユーザーのメールアドレス(`iapUserEmail()`)と言語切替を表示する。
...
- **パイプライン状態レール** — 「① 収集 → ② 生成 → ③ 投稿」の 3 ステージを矢印で結んだ帯。収集は `runs` の最新 `collect`、生成は最新の `generate_*` の成否と開始時刻を、投稿は最新の投稿(post)のステータスバッジとタイトルを表示する。データソースは既存の `getRecentRuns()` / `getRecentPosts()` で、追加の読み取りはない
...
- **自動実行設定カードとコンテンツ生成カード**(広い画面では横並び) — 生成カードは「① 収集」「② 日次生成」「② 週次生成」「② 月次生成」の 4 ボタン(`ActionButton` + `runJobNow(...)`)。押すと対応する Cloud Run Job を起動する(5.11 と同じ仕組み)。日次は各チャネルへ自動投稿、週次・月次は下書きを作成 → 下書きページで承認。スケジュール(06:00 収集 / 08:00 日次 / 月曜 週次 / 月初 月次)を待たずに今すぐ走らせたいとき用。実行状況は下の「ジョブ実行履歴」で確認する
- **承認待ちの下書き**(先頭 5 件) — 周期チップ(`daily` 等)+タイトル。クリックで下書き編集へ
- **直近の投稿**(10 件) — 投稿ステータスとチャネルごとのステータスバッジ
- **ジョブ実行履歴**(10 件) — `runs` の `jobType`・成否・開始時刻・統計 JSON・先頭エラー(80 文字まで)。成否は「成功 / 失敗」(`dashboard.runOk` / `dashboard.runFailed`)の色ドット付きテキストで表示する
...
リトライは `retryChannel(postId, channel)` → pipeline-api の `POST /api/posts/{id}/retry-channel`。失敗した 1 チャネルだけを再実行するので、成功済みチャネルに二重投稿されることはない。結果は ActionButton の隣に `✓` またはエラー本文で表示される(9 章)。
...
- **ジョブ手動実行**: `collect` `generate_daily` `generate_weekly` `generate_monthly` `cleanup_drafts` `refresh_threads_token` `seed` の 7 ボタン(cleanup / seed / token 更新も含む全ジョブ。日次〜月次だけならダッシュボードのカードが便利)。`runJobNow()` → pipeline-api の `POST /api/jobs/{name}/run` は **202(受付)を即返し**、pipeline-api が対応する **Cloud Run Job を起動する**(スケジューラ経由と同じジョブ・同じ実行環境。`05-pipeline-api.md` 6a 節)。進行結果はダッシュボードのジョブ実行履歴で確認する
...
一方、クリックや入力に反応するコードはブラウザで動く必要がある。ファイル先頭に `'use client'` と書いたものだけがブラウザに JavaScript として配送される(Client Component)。admin でこの宣言を持つのは **`AppShell.tsx` / `DraftEditor.tsx` / `ActionButton.tsx` / `LocaleSwitcher.tsx` / `NavLink.tsx` の 5 ファイルだけ**(NavLink は `usePathname()` で現在 URL を読むため。AppShell はモバイルドロワーの開閉状態を `useState` で持つため)。`ui.tsx` と `icons.tsx` には宣言がなく、サーバ・ブラウザ両方から import されて双方で使える。原則: **表示とデータ取得はサーバ、対話だけブラウザ**。
...
- **ActionButton**(リトライ・ジョブ実行系): 実行中はラベル横に回転スピナーが出て連打不可。完了すると隣に、成功なら緑の `✓`、失敗なら **赤字でレスポンス本文の先頭 200 文字** を表示する。表示は次の操作まで残る。自動リトライはしない
