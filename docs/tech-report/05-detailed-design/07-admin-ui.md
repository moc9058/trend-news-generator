...
> 対象コード時点: コミット 35f05ef + 未コミット変更 / 最終更新: 2026-07-16(レポート実行詳細の LLM トレースカードと `langsmith.ts` を追記)
...
| | `ResearchTrace.tsx` | レポート実行詳細の LLM トレースカード(サーバ側。LangSmith を読み戻す。Suspense 境界の内側) |
| | `TraceTree.tsx` | トレース木の描画。行を開くとプロンプト本文・生成結果が出る(ブラウザ側で動く) |
...
| | `langsmith.ts` | LangSmith REST の読み取り専用クライアント(**Firestore 以外を読む唯一の data 層**。6.9 節) |
...
このほか research 2 画面(`/research` 一覧・`/research/[id]` 実行詳細)がある(詳細は [10-research-agent.md](10-research-agent.md))。実行詳細には**2 つの可視化カード**が並ぶ — 出所が違うので役割も違う:

| カード | 部品 | データ源 | 見えるもの |
|---|---|---|---|
| Agent flow | `ResearchFlow.tsx`(`@xyflow/react`) | Firestore `events` + run doc | 6 フェーズ+コネクタ+言語別 localize ノードの DAG。**形は `PHASES` 定数に固定**で、events を数えて状態・回数・コスト・トークンを載せる |
| LLM トレース | `ResearchTrace.tsx` → `TraceTree.tsx` | **LangSmith(REST)** | 呼び出しごとの**プロンプト本文・生成結果・レイテンシ・実際の入れ子**。Firestore には無いものだけ([10-research-agent.md](10-research-agent.md) §6.8) |

トレースカードは `langsmith-api-key` シークレットが無ければ丸ごと消える(pipeline のトレーシングと同じスイッチ)。
...
**進行中レポートのバナー**: レポートの下書き `posts` は Research Agent の run が最終 `review` フェーズまで走り切って初めて生成される([10-research-agent.md](10-research-agent.md))。それまで数分間この画面には何も出ないため、手動でレポートを依頼した利用者が「消えた」と誤解しないよう、下書き一覧の**上に**進行中の run を表示する。データは `getInProgressResearchRuns()`(`getResearchRuns()` の直近取得を JS で絞る = 複合インデックス不要)で、`postId` 未設定かつ status が `queued` / `running` / `awaiting_plan_approval` / `failed` / `budget_exhausted` の run。draft が出来た瞬間(`review._handoff` が `postId` を立てる)にこのリストから外れ、下の通常の下書き一覧へ移る。行はステータス・テーマ(`/research/[id]` へリンク)・フェーズ・コストで、`awaiting_plan_approval` には「計画の承認が必要」の注記が付く。**自動更新**は `AutoRefresh.tsx`(Client Component。`router.refresh()` を 5 秒間隔で呼ぶだけ)で、`running` 系が 1 件でもある間だけポーリングを有効化する(7.4 節)。
...
### 6.9 `langsmith.ts` — LangSmith REST の読み取り(Firestore 以外を読む唯一の data 層)

4 章の「Firestore 直書き / pipeline-api 経由」の二分法に**入らない**第三の経路。書き込みではなく表示のための読み取りなので pipeline-api を挟む理由が無く、かといって Firestore にはデータが無い(トレースの正は LangSmith にしかない — [10-research-agent.md](10-research-agent.md) §6.8)。

| 関数 | 何をするか |
|---|---|
| `langsmithEnabled()` | `LANGSMITH_API_KEY` の有無。ページ側はこれで Suspense 境界ごと出し分ける |
| `getResearchTrace(runId)` | プロジェクト UUID 解決 → runId メタデータでルート実行 → `trace_id` で全スパン → `dotted_order` 順の木にして返す。**失敗は全て `null`** |

- **`langsmith` npm パッケージは入れず素の `fetch`**(Python 側が内部 ABI で `langsmith<1` に pin されている問題を admin に持ち込まない。使う API は 2 本だけ)
- **`null` に潰す設計**: トレーシング off・トレース未生成・LangSmith 不達を呼び出し側は区別しない。third-party の障害で管理画面が 500 になってはいけないため。カードが消えるだけで他のセクションは何も変わらない
- **Suspense 境界の内側に置く**(`page.tsx`)。Firestore 由来のカードを外部 API の往復で待たせない
- プロンプト・生成文は 4000 字で切ってからブラウザへ送る(全文は「LangSmith で開く」リンク)。トレース自体も 300 スパンで打ち切り、**切ったことを UI に出す**(黙って落とすと「全部見えている」と誤読される)
...
ただし 1・2 はどちらも**利用者の操作を起点**にした再描画で、バックエンドが後から Firestore を書いても画面は勝手には更新されない。数分かかるレポート run の進捗のように、操作なしで更新を反映したい画面には**第 3 の手段**として `AutoRefresh.tsx`(5.3 節)を使う — force-dynamic な画面に対し `router.refresh()` を定期的に呼ぶだけの Client Component で、実質サーバ側ポーリングになる(admin はクライアント Firebase SDK を持たないので `onSnapshot` は使わない。ストリーミングが要るチャット送信だけは 7.6 の route handler + SSE)。無駄なポーリングを避けるため、進行中の run がある間だけ `enabled` で有効化する。
