...
lease and run the Research Harness on each (design §4.6, §6.1).

Deployed with **--max-retries=1** (unlike the posting jobs): this job produces a
draft only, and the lease + resume guarantee that a task retry re-acquires the
same run and continues from its last completed phase rather than double-executing
(design §6.3). `worker_id` is CLOUD_RUN_EXECUTION, stable across task retries.
"""
